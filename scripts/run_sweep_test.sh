...
CUDA_VISIBLE_DEVICES=0 python main.py -mode sweep_test -config configs/"$dataset"/sweep.yaml -sweep_name "$sweep_name"  -sweep_id "$sweep_id" -sweep_count "$sweep_count" &
CUDA_VISIBLE_DEVICES=0 python main.py -mode sweep_test -config configs/"$dataset"/sweep.yaml -sweep_name "$sweep_name"  -sweep_id "$sweep_id" -sweep_count "$sweep_count" &

for i in 1 2 3 4 5 6 7
do
  CUDA_VISIBLE_DEVICES=$i python main.py -mode sweep_test -config configs/"$dataset"/sweep.yaml -sweep_name "$sweep_name"  -sweep_id "$sweep_id"  -sweep_count "$sweep_count" &
  CUDA_VISIBLE_DEVICES=$i python main.py -mode sweep_test -config configs/"$dataset"/sweep.yaml -sweep_name "$sweep_name"  -sweep_id "$sweep_id"  -sweep_count "$sweep_count" &
