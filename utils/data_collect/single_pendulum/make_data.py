...
def render(theta):
    ...
    pendulum_im = Image.open('utils/data_collect/single_pendulum/pendulum.png')
...
def make_stationary_data(data_filepath, num_frm):

    seq_filepath = os.path.join(data_filepath, str(-1))
    mkdir(seq_filepath)

    states = np.zeros((num_frm, 2))
    for k in range(num_frm):
        im = render(states[k,0])
        im.save(os.path.join(seq_filepath, str(k)+'.png'))



if __name__ == '__main__':
    data_filepath = 'data/single_pendulum'
